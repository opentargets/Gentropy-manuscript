...
This directory uses `renv` for R package management to ensure reproducible analysis.

## Setup

The R environment has been initialized with all required packages. When you open R in this directory, `renv` will automatically activate.

## Required Packages

The following packages are installed in this environment:

- **Visualization**: `circlize`, `ggplot2`, `cowplot`, `patchwork`, `RColorBrewer`, `png`
...
Simply run your R scripts as usual. The environment will be automatically activated:
...
- `renv.lock` - Lockfile containing all package versions (commit this to version control)
