"""Enrichment statistics shared by the R2-MJ-2 optimism-bias notebooks.

`or_rs` and `support_mask` are the same implementations verified in
`01_build_pair_tables.ipynb`, where they reproduce the published OR = 3.62 (all GWAS) and
OR = 10.29 / RS = 4.84 (PAV + 2-5 therapeutic areas) from the pair-level master table.
`or_rs` mirrors `gentropy.method.drug_enrichment_from_evid.chemblDrugEnrichment
.drug_enrichemnt_from_evidence` exactly: Fisher's exact odds ratio, relative success as a risk
ratio, Woolf log-scale confidence intervals with z = 1.96.

Notebooks 02-04 import from here so no definition can drift between phases.
"""
