...
class RepresentativeStudyManifestStep:
    ...
    def __init__(
        ...
        self.include_ancestries: list[str] = include_ancestries if include_ancestries is not None else ["nfe"]
        self._run()
    ...
    def _write_delta(
        ...
        deprecated_mask = prev["studyId_1"].isin(deprecated_studies) | prev["studyId_2"].isin(deprecated_studies)
        deprecated_pairs = prev[deprecated_mask].copy()
        deprecated_pairs["deprecated_because"] = deprecated_pairs.apply(
            lambda r: (
                f"{r['studyId_1']} replaced" if r["studyId_1"] in deprecated_studies else f"{r['studyId_2']} replaced"
            ),
            axis=1,
        )
    ...
    def _load_and_filter_studies(self) -> pd.DataFrame:
        ...
        study_pd["ld_ancestry"] = study_pd["ldPopulationStructure"].apply(self._safe_infer_ancestry)
    ...
    def _merge_and_apply_h2_filters(self, study_pd: pd.DataFrame, h2_pd: pd.DataFrame) -> pd.DataFrame:
        ...
        merged["h2_z_sq"] = (merged["h2"] / merged["h2_se"].replace(0, float("nan"))) ** 2

        merged = merged[
            (merged["h2"] > self.min_h2) & (merged["h2"] < self.max_h2) & (merged["h2_z_sq"] >= self.min_h2_z_squared)
        ]

        lambda_ok = merged["lambda_gc"].isna() | (
            (merged["lambda_gc"] >= self.min_lambda_gc) & (merged["lambda_gc"] <= self.max_lambda_gc)
        )
    ...
    def _select_representatives(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        exploded = df.explode("_disease_list").rename(columns={"_disease_list": "diseaseId"})
        exploded = exploded[exploded["diseaseId"].notna() & (exploded["diseaseId"] != "")]
        ...
        exploded["rank_in_cell"] = (
            exploded.groupby(["diseaseId", "ld_ancestry"])["neff"].rank(method="first", ascending=False).astype(int)
        )
    ...
    def _generate_pairs(self, representatives: pd.DataFrame) -> pd.DataFrame:
        ...
        studies_per_ancestry: dict[str, list[str]] = (
            representatives.groupby("ld_ancestry")["studyId"].apply(lambda x: sorted(x.unique().tolist())).to_dict()
        )

        rows: list[dict[str, str]] = []
        for ancestry, study_ids in sorted(studies_per_ancestry.items()):
            n_pairs = len(study_ids) * (len(study_ids) - 1) // 2
            logger.info("Ancestry %s: %d studies → %d pairs", ancestry, len(study_ids), n_pairs)
