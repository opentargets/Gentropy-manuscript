...
gene_pleio    <- read.csv(file.path(data_dir, "Fig4A_stats_gene_pleiotropy.csv"),
                          sep = "\t", row.names = 1)
variant_pleio <- read.csv(file.path(data_dir, "Fig4A_stats_variant_pleiotropy-r1.csv"),
                          sep = "\t", row.names = 1)
...
# ---- R2 minor comment 3: plot odds ratios, not log odds ratios -------------
# The in-text values and the Figure 4 caption were converted to odds ratios per doubling of gPS;
# only this axis still showed log-odds. odds_ratio / ci_lower / ci_upper are already columns of the
# input table (they are exp() of the log columns, written by
# `04_gene_pleiotropy_by categories.ipynb`), so this is a column swap, not a recomputation.
#
# Geometry is preserved exactly. All layout constants below are still derived in log space and then
# exponentiated, because position on a log-scaled OR axis is proportional to log(OR) = log_odds_ratio.
# Point spacing and interval widths are therefore identical to the published panel; only the axis
# furniture changes.
x_lo   <- min(plot_data$log_ci_lower, na.rm = TRUE)
x_hi   <- max(plot_data$log_ci_upper, na.rm = TRUE)
x_span <- x_hi - x_lo
x_col1 <- exp(x_hi + x_span * 0.22)   # right edge of "Genes" column
x_col2 <- exp(x_hi + x_span * 0.42)   # right edge of "In set" column
y_hdr  <- nlevels(plot_data$label) + 1.2
x_axis_lo <- exp(x_lo - 0.05 * x_span)
x_axis_hi <- exp(x_hi + 0.05 * x_span)

# Symmetric about 1 on a log scale: 0.8 and 1.25 are reciprocals, as are 0.9 and 1.111 (~1.1).
or_breaks <- c(0.8, 0.9, 1, 1.1, 1.25, 1.5)
or_labels <- c("0.8", "0.9", "1", "1.1", "1.25", "1.5")

# Where -Inf resolves to: the coord_cartesian range after the default 5% continuous expansion,
# computed in log space because the scale is log10.
x_hdr_anchor <- 10^(log10(x_axis_lo) - 0.05 * (log10(x_axis_hi) - log10(x_axis_lo)))

p_c <- ggplot(plot_data, aes(x = odds_ratio, y = label, colour = sig_label)) +
  geom_vline(xintercept = 1, colour = col_vline, linetype = "dashed",
             linewidth = 0.3) +
  ...
  geom_text(aes(x = x_col2, y = label, label = pct_label),
            hjust = 1, size = text_size / .pt, color = text_colour,
            inherit.aes = FALSE) +
  # x = -Inf cannot be log-transformed (log(-Inf) is NaN and the layer is silently dropped), so the
  # panel minimum is passed explicitly. -Inf resolves to the *expanded* lower bound, not to
  # x_axis_lo, so the default 5% continuous expansion is reproduced here in log space. That puts the
  # header on exactly the pixel it occupies in the published panel.
  annotate("text", x = x_hdr_anchor, y = y_hdr, label = "Gene/Target Set",
  ...
  scale_x_log10(breaks = or_breaks, labels = or_labels) +
  coord_cartesian(xlim = c(x_axis_lo, x_axis_hi), clip = "off") +
  labs(x = "Odds ratio per doubling of gPS", y = NULL) +
  base_theme +
  theme(
    axis.text.x          = element_text(size = text_size, face = "plain", color = text_colour),
    legend.position      = c(1, 0),
...
png_file <- file.path(out_dir, "figure_4_final-r1.pdf")
